...
public class OCR {
	...
	public static String ocrCode(String imgFilePath, int limitCodeLen) {
		...
		String html;
		try {
			html = HttpClientUtil.upload(HttpConfig.custom().client(client).url(apiUrl)
					.files(new String[] { imgFilePath }, "ocrfile", true).map(getParaMap()));
	...
	public static String ocrCode4Net(String imgUrl, int limitCodeLen) {
		...
		Header[] headers = HttpHeader.custom()
				.userAgent("Mozilla/5.0 (Windows NT 5.1; zh-CN; rv:1.9.1.3) Gecko/20100101 Firefox/8.0").build();
	...
	public static void main(String[] args) throws HttpProcessException, IOException {
		...
		String code3 = ocrCode4Net(HttpConfig.custom().url(url2),
				System.getProperty("user.dir") + System.getProperty("file.separator") + "123.png", 5);
